        ...
        for (j = 0; j < m; j++)
        {
            if (arr[i][j] == search[0] || arr[i][j] == '*' || arr[i][j] == '+')
            {
                for (k = 0; k < search.length(); k++)
                {
                    ...
                    {
                        int loop = k + 1;
                        for (int l = k + 1; l < search.length(); k++)
                        {
...
void searchleft(char **arr, int n, int m, string search)
{
    int i, j, k = 0, l;
    int counter = 0;
    bool found = false;
        ...
        for (j = m - 1; j >= 0; j--)
        {
            if (arr[i][j] == search[0] || arr[i][j] == '*' || arr[i][j] == '+')
            {
                for (k = 0; k < search.length(); k++)
                {
                    ...
                    if (j - k < 0)
                        goto end;
                    if (arr[i][j - k - 1] != search[k + 1] || arr[i][j - k - 1] != search[k + 2] || arr[i][j - k - 1] != search[k + 3] || arr[i][j - k - 1] != search[k + 4] || arr[i][j - k - 1] != search[k + 5])
                        goto end;
                    if (arr[i][j - k] == '+' || arr[i][j - k] == '?')
                    {
                        found = true;
                    ...
                    if (arr[i][j - k] == '*')
                    {
                        if (arr[i][j - k - 1] == search[search.length() - 1] || arr[i][j - k - 3] == search[search.length() - 1] || arr[i][j - k - 2] == search[search.length() - 1] || arr[i][j - k - 4] == search[search.length() - 1] || arr[i][j - k - 5] == search[search.length() - 1])
                        {
